...
def create_dim_date(sales_df):
    ...
    # Convert date to YYYY-MM-DD string format for CSV
...
def calculate_net_profit(marketing_df, shipping_df):
    ...
    if subcategories_df is not None:
        # Detect subcategory column in products
        prod_subcat_col = next((col for col in ['subcategory_id', 'subcat_id', 'sub_category_id'] 
                                if col in products_df.columns), None)
    ...
    # Net Profit (matching transform.py formula)
    ...
    # Select final columns for fact_sales (matching transform.py structure)
...
# ===============================================================
# 11. LEGACY INVOICES (OCR) - OPTIONAL
# ===============================================================
def process_legacy_invoices(manual=False):
    ...
    print("✓ Legacy invoices processing skipped (optional feature)")
    return legacy_df


# ===============================================================
# 12. CALCULATE MARKETING ROI
# ===============================================================
...
def save_all_tables(dim_product, dim_store, dim_customer, dim_date, fact_sales, marketing_roi):
    ...
    for filename, df in tables.items():
        if df is not None and not df.empty:
            ...
            try:
                # Special handling for dim_date - ensure date is string
